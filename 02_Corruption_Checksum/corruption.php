...
function checksumSpreadsheet($sheet)
{
    $checksum = 0;
    
    foreach ($sheet as $row) {
        // Could also sort cells here and take first and last
        $min = MAX_VALUE;
        $max = MIN_VALUE;
...
function parseInput($input)
{
    ...
    foreach ($textRows as $textRow) {
        $cells = preg_split('/\s+/', trim($textRow));
        $sheet[] = $cells;
    }
...
$sheet = parseInput($testInput);
assert(checksumSpreadsheet($sheet) == 18);

$sheet = parseInput($puzzleInput);
$checksum = checksumSpreadsheet($sheet);
print "$checksum\n\n";
